...
/*   Updated: 2026/03/21 15:25:00 by elara-va         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	initialize_next_nodes(t_pipes **pipe_node, t_pids **pid_node,
...
int	create_pipe_and_pib_lists(t_pipes **pipe_list, t_pids **pid_list,
	int nbr_of_children)
{
	int		child_count;
	t_pipes	*pipe_node;
	...
	child_count = 1;
	if (initialize_heads(pipe_list, pid_list, child_count) != 0)
		return (1);
	pipe_node = *pipe_list;
	pid_node = *pid_list;
	while (++child_count <= nbr_of_children)
	{
		if (initialize_next_nodes(&pipe_node, &pid_node, child_count,
				nbr_of_children) != 0)
		{
